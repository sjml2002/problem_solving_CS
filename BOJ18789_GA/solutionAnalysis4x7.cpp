    ...
    void calc_visUniquePath() {
        for(int score=0; score<=MAXNUM; score++) {
            if (possibleRepresent[score].size() == 1) {
                //possibleRepresent[score][0] 은 score에서 unique한 path임.
                for(int i=0; i<possibleRepresent[score][0].size(); i++) {
                    int y = possibleRepresent[score][0][i].first;
                    int x = possibleRepresent[score][0][i].second;
                    visUniquePath[y][x]++;
                }
            }
        }
