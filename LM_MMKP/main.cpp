...
static const std::string RESULTS_CSV = RESULTS_DIR + "LM_MMKP_original.csv";
// Directory where standard instance files live (adjust path as needed).
...
int main() {
    ...
    if (!solutionsLoaded) {
        std::cerr << "[main] Warning: could not load " << SOLUTIONS_CSV
    ...
    if (!fout.is_open()) {
        std::cerr << "[main] Failed to open output file: " << RESULTS_CSV << std::endl;
        return 1;
    }

    fout << "instance,N,M,our_value,known_optimum,percent_diff\n";
    ...
    for (const std::string& fileName : FILE_LIST) {
        ...
        if (!ok) {
            std::cerr << "[main] Failed to read instance: " << fileName << std::endl;
            ++failCount;
            continue;
        }

        SolutionResult result = solve(instance);
        ++successCount;
        ...
        if (it != solutionMap.end()) {
            ...
            if (knownOpt != 0) {
                // percent_diff = (our_value - known_optimum) / known_optimum * 100
        ...
        std::cout << "[main] " << fileName
                  << " N=" << instance.N << " M=" << instance.M
                  << " ourValue=" << result.bestValue
                  << " knownOpt=" << knownOptStr
                  << " percentDiff=" << percentDiffStr << std::endl;
    }

    fout.close();

    std::cout << "\n[main] Done. Success: " << successCount
              << ", Failed: " << failCount
              << ", Matched with solutions.csv: " << matchedCount
              << ", Total: " << FILE_LIST.size() << std::endl;
    std::cout << "[main] Results written to: " << RESULTS_CSV << std::endl;
